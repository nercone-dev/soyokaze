...
fn http1() {
    ...
    group.bench("parse_start_line (request)", || h1::parse_start_line(opaque("GET /index.html HTTP/1.1")));
    group.bench("parse_start_line (status)", || h1::parse_start_line(opaque("HTTP/1.1 404 Not Found")));

    let request = Message::request(Method::GET, "/index.html", Version::V1_1);
    group.bench("write_start_line (request)", || {
        let mut out = BytesMut::new();
        let _ = h1::write_start_line_into(opaque(&request), &mut out);
        out
    });

    let mut group = Group::new("http/1 field lines");
    group.bench("parse_header_line", || h1::parse_header_line(opaque("Content-Type: text/html; charset=utf-8")));

    let headers = request_headers();
    let size = h1::headers_size(&headers) as usize;
    group.throughput("write_headers (5 fields)", size, || {
        let mut out = BytesMut::new();
        h1::write_headers_into(opaque(&headers), HeaderCase::Title, &mut out).unwrap();
        out
    });
    ...
    let lines: Vec<String> = headers
        ...
        .collect();
    group.throughput("parse_headers (5 fields)", size, || h1::parse_headers(opaque(lines.clone())));

    let block: Vec<u8> = lines.iter().flat_map(|line| format!("{line}\r\n").into_bytes()).collect();
    group.throughput("parse_header_block (5 fields)", size, || h1::parse_header_block(opaque(&block), 100));

    let mut framed = block.clone();
    framed.extend_from_slice(b"\r\n");
    group.bench("block_end (5 fields)", || h1::block_end(opaque(&framed), &mut 0));
    ...
    group.throughput("parse request head (5 fields)", head, || {
        let message = h1::parse_start_line_bytes(opaque(request_line.as_slice()));
        (message, h1::parse_header_block(opaque(&block), 100))
    });
    ...
    let size = 15 + h1::headers_size(response.headers.as_ref().unwrap()) as usize;
    group.throughput("write response head (4 fields)", size, || {
        let mut out = BytesMut::with_capacity(256);
        let _ = h1::write_start_line_into(opaque(&response), &mut out);
        out.extend_from_slice(b"\r\n");
        let _ = h1::write_headers_into(response.headers.as_ref().unwrap(), HeaderCase::Title, &mut out);
        out.extend_from_slice(b"\r\n");
        out
    });
    ...
    group.throughput("encode_chunk (4 KiB)", chunk.len(), || h1::encode_chunk(opaque(&chunk)));

    let encoded = h1::encode_chunk(&chunk);
    group.bench("decode_chunk (4 KiB, no copy)", || h1::decode_chunk(opaque(&encoded)));
    group.bench("parse_chunk_size", || h1::parse_chunk_size(opaque(b"1000;name=value\r\n")));
}
...
fn pseudo_headers() {
    ...
    let fields = common::fields(&request).expect("the fixture request did not encode");
    let size: usize = fields.iter().map(|field| field.name.len() + field.value.len()).sum();

    let mut group = Group::new("pseudo-headers");
    group.throughput("common::fields (request)", size, || common::fields(opaque(&request)));
    group.throughput("common::message (request)", size, || common::message(opaque(&fields), Version::V2_0));
    ...
    group.bench("common::message (response)", || common::message(opaque(&response), Version::V2_0));
}
...
fn http2() {
    ...
    group.throughput("decode DATA (16 KiB)", 16_384, || {
        Frame::decode(header, opaque(&encoded[h2::FRAME_HEADER_SIZE..]))
    });

    let payload = bytes::Bytes::copy_from_slice(&encoded[h2::FRAME_HEADER_SIZE..]);
    ...
    group.bench("decode SETTINGS (4 pairs)", || Frame::decode(header, opaque(&encoded[h2::FRAME_HEADER_SIZE..])));
...
fn http3() {
    ...
    for (name, value) in [("1 octet", 37u64), ("2 octets", 15_293), ("4 octets", 494_878_333), ("8 octets", h3::MAXIMUM_VARINT)] {
        let mut out = BytesMut::with_capacity(16);
        group.bench(&format!("encode_varint ({name})"), || {
            out.clear();
            h3::encode_varint(&mut out, opaque(value));
        });

        let mut encoded = BytesMut::new();
        h3::encode_varint(&mut encoded, value);
        group.bench(&format!("decode_varint ({name})"), || h3::decode_varint(opaque(&encoded)));
    }
    ...
    let encoded = data.encode();
    group.throughput("parse_frame DATA (16 KiB)", 16_384, || {
        let mut buffer = BytesMut::from(&encoded[..]);
        h3::parse_frame(&mut buffer)
    });
}
...
fn digests() {
    ...
    group.bench("accept_key", || websocket::accept_key(opaque("dGhlIHNhbXBsZSBub25jZQ==")));
}
